...
export default function Detail() {
  ...
  const nextSlide = () => {
    const newIndex = currentImgIndex === champion?.skins.length - 1 ? 0 : currentImgIndex + 1;
  ...
  const prevSlide = () => {
    const newIndex = currentImgIndex === 0 ? champion?.skins.length - 1 : currentImgIndex - 1;
    setCurrentImgIndex(newIndex);
    setStyle({
      transform: `translateX(-${newIndex}00%)`,
      transition: `all 0.4s ease-in-out`,
  ...
  if (isLoading || isFetching) {
    return (
      <div className="font-bold text-center text-2xl text-white">Loading...</div>
    )
  }
    ...
    <div className="flex justify-center text-white">
      <div className="relative">
        ...
        <div className="absolute w-full flex justify-between top-1/2">
          ...
          <button className="text-white text-2xl cursor-pointer" onClick={nextSlide}>
            {"▷"}
          </button>
        </div>
      </div>
